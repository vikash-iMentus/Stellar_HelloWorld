...
async function main() {
    ...
    // console.log("contract",contractIdObj);
    // console.log(SorobanClient.xdr.ScVal.scvObject(SorobanClient.xdr.ScObject.scoBytes("GD5BPDLGED7ELAJN5D7JS5OFBLK3HT6BF5PBZFTFGDF5HTKYTER5VN4J")));
    ...
    // console.log("SEQUENCE: ", typeof(strSeq));
   


    let invokerAccount = await server.getAccount(invokerKeypair.publicKey()) // In stellar-sdk: // const invokerAccount = await server.loadAccount(invokerKeypair.publicKey());
    ...
    // **************************************************************************
     let opts = {
        ...
        // function: SorobanClient.xdr.HostFunction.hostFunctionTypeInstallContractCode(),
    ...
    let arg1 = new Address(cId).toScVal();
    // let contractFnNameObj = SorobanClient.xdr.ScVal.scvSymbol("test_world"); // let's say we want to invoke hello method
    opts.parameters.push(arg1); // 2. Push Function Name
     
    // let arg2 = SorobanClient.xdr.CreateContractArgs.contractId("GBOT5HNYQASVN2HFLTMSYPSOM2O3WAIFUKXXSF3RBSVXKY4BCL4NOUHN");
    let buf = Buffer.from(cId, "utf-8");
    ...
    console.log("Source: 999999999999999999999999999", sourceVar);
    let arg2 = new SorobanClient.xdr.CreateContractArgs(conId,sourceVar);
    // console.log("Arg2: ", arg2);
    console.log("parameters1: ", opts.parameters);
    opts.parameters.push(arg2);
    console.log("parameters2: ", opts.parameters);
    // let arg3 = SorobanClient.xdr.CreateContractArgs.source(SorobanClient.xdr.ScContractCode.sccontractCodeToken());
    // opts.parameters.push(arg3);
    // let arg4 = SorobanClient.xdr.InstallContractCodeArgs.
    // // SorobanClient.xdr.ScValType.SCO_ADDRESS(GD5BPDLGED7ELAJN5D7JS5OFBLK3HT6BF5PBZFTFGDF5HTKYTER5VN4J);
    // // console.log(SorobanClient.xdr.ScValType.SCO_ADDRESS(GD5BPDLGED7ELAJN5D7JS5OFBLK3HT6BF5PBZFTFGDF5HTKYTER5VN4J));
    ...
    // contractMethodParameterObj = SorobanClient.xdr.ScVal.scvU32(42); // u64 - working ref
    // opts.parameters.push(contractMethodParameterObj); // 4. Push Function Argument 2
    // **************************************************************** 
    // Footprint Way 02 // For reference purposess
    ...
    // let check = opts.footprint.readOnly().push(SorobanClient.xdr.LedgerKey.contractData(new SorobanClient.xdr.LedgerKeyContractData({ contractId: Buffer.from(process.env.HELLO_CONTRACT_ID, 'hex'), key: contractCodeFootprint })))
    // console.log("check",contractCodeFootprint);
    // opts.footprint.readWrite().push(SorobanClient.xdr.LedgerKey.contractData(new SorobanClient.xdr.LedgerKeyContractData({ contractId: Buffer.from(process.env.HELLO_CONTRACT_ID, 'hex'), key: incrementStateFootprint })))
    ...
    // let arg1 = SorobanClient.xdr.ScVal.scvSymbol("World");
    // let arg2 = SorobanClient.xdr.ScVal.scvU32(42);
    // let arr = [arg1,arg2];
    ...
    // console.log("rang",hello_transaction);
    ...
    // hello_transaction.sign(invokerKeypair);
    // console.log('Transaction has been signed using invokerKeypair.');

    // // // Finally, send the singed tx obj to the servers.
    try {
        let response = await server.sendTransaction(hello_transaction); // Soroban server do not have a submitTransaction endpoint
